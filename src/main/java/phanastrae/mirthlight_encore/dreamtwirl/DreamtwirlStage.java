...
public class DreamtwirlStage {
    ...
    public void clear(ServerWorld serverWorld) {
        ...
        if(DTWA == null) {
            MirthlightEncore.LOGGER.warn("Someone tried to clear a non Dreamtwirl region???? how???");
            return;
        }
        ...
        PalettedContainer<BlockState> barrierBlockStateContainer = new PalettedContainer<>(Block.STATE_IDS, MirthlightEncoreBlocks.DREAMTWIRL_BARRIER.getDefaultState(), PalettedContainer.PaletteProvider.BLOCK_STATE);
        ...
        PalettedContainer<RegistryEntry<Biome>> biomeContainer = new PalettedContainer<>(
                biomeRegistry.getIndexedEntries(), biomeRegistry.entryOf(MirthlightEncoreBiomes.DREAMTWIRL), PalettedContainer.PaletteProvider.BIOME
        );
